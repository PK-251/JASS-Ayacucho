...
@section('content')
    ...
    <div class="topbar">
        <div>
            ...
            <div class="page-subtitle">{{ ucfirst(now()->locale('es')->translatedFormat('F Y')) }} · Periodo activo</div>
        </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-xl-3 col-md-6">
            <x-metric-card label="Total Usuarios" icon="users" badge="Activos">{{ $totalUsuarios }}</x-metric-card>
        </div>
        <div class="col-xl-3 col-md-6">
            <x-metric-card label="Al Dia" icon="user-check" tone="success" valueClass="success" :badge="($totalUsuarios ? round($alDia * 100 / $totalUsuarios, 1) : 0) . '%'">{{ $alDia }}</x-metric-card>
        </div>
        <div class="col-xl-3 col-md-6">
            <x-metric-card label="Morosos" icon="user-x" tone="danger" valueClass="danger" badge="Requieren atencion" badgeClass="badge-danger-soft">{{ $morosos }}</x-metric-card>
        </div>
        <div class="col-xl-3 col-md-6">
            <x-metric-card label="Balance Mayo" icon="wallet" :tone="$balanceEsDeficit ? 'danger' : 'default'" :valueClass="$balanceEsDeficit ? 'danger' : ''" :badge="$balanceEsDeficit ? 'Deficit' : 'Neto'" :badgeClass="$balanceEsDeficit ? 'badge-danger-soft' : 'badge-soft'">S/{{ number_format(abs($balanceMes), 2) }}</x-metric-card>
        </div>
    </div>
